import Link from "next/link";

export default function ArticleList() {
  return (
    <>
      <ul className="w-[41.7vw]">
        <li className="py-6 pl-8 pr-8 border-t-1 border-b-1 border-[var(--color-secondary-gray)]">
          <Link href="/nikki/1" className="block">
            <dl>
              <dt className="flex justify-start items-center gap-4">
                <span className="text-xs text-[var(--color-text-primary)] leading-[1]">
                  2025-00-00
                </span>
                <span className="text-xxs text-white px-2 py-1 bg-[var(--color-primary)] font-quaternary rounded-xl leading-[0.9] font-light tracking-[0.05rem]">
                  #勉強記録
                </span>
              </dt>
              <dd className="mt-4 text-md text-[var(--color-text-primary)] font-secondary leading-[1.6] tracking-[0.2rem] truncate w-[35vw]">
                日記のタイトルが入ります。
              </dd>
            </dl>
          </Link>
        </li>
        <li className="py-6 pl-8 pr-8 border-b-1 border-[var(--color-secondary-gray)]">
          <Link href="/nikki/1" className="block">
            <dl>
              <dt className="flex justify-start items-center gap-4">
                <span className="text-xs text-[var(--color-text-primary)] leading-[1]">
                  2025-00-00
                </span>
                <span className="text-xxs text-white px-2 py-1 bg-[var(--color-primary)] font-quaternary rounded-xl leading-[0.9] font-light tracking-[0.05rem]">
                  #勉強記録
                </span>
              </dt>
              <dd className="mt-4 text-md text-[var(--color-text-primary)] font-secondary leading-[1.6] tracking-[0.2rem] truncate w-[35vw]">
                日記のタイトルが入ります。
              </dd>
            </dl>
          </Link>
        </li>
        <li className="py-6 pl-8 pr-8 border-b-1 border-[var(--color-secondary-gray)]">
          <Link href="/nikki/1" className="block">
            <dl>
              <dt className="flex justify-start items-center gap-4">
                <span className="text-xs text-[var(--color-text-primary)] leading-[1]">
                  2025-00-00
                </span>
                <span className="text-xxs text-white px-2 py-1 bg-[var(--color-primary)] font-quaternary rounded-xl leading-[0.9] font-light tracking-[0.05rem]">
                  #勉強記録
                </span>
              </dt>
              <dd className="mt-4 text-md text-[var(--color-text-primary)] font-secondary leading-[1.6] tracking-[0.2rem] truncate w-[35vw]">
                日記のタイトルが入ります。
              </dd>
            </dl>
          </Link>
        </li>
        <li className="py-6 pl-8 pr-8 border-b-1 border-[var(--color-secondary-gray)]">
          <Link href="/nikki/1" className="block">
            <dl>
              <dt className="flex justify-start items-center gap-4">
                <span className="text-xs text-[var(--color-text-primary)] leading-[1]">
                  2025-00-00
                </span>
                <span className="text-xxs text-white px-2 py-1 bg-[var(--color-primary)] font-quaternary rounded-xl leading-[0.9] font-light tracking-[0.05rem]">
                  #勉強記録
                </span>
              </dt>
              <dd className="mt-4 text-md text-[var(--color-text-primary)] font-secondary leading-[1.6] tracking-[0.2rem] truncate w-[35vw]">
                日記のタイトルが入ります。
              </dd>
            </dl>
          </Link>
        </li>
        <li className="py-6 pl-8 pr-8 border-b-1 border-[var(--color-secondary-gray)]">
          <Link href="/nikki/1" className="block">
            <dl>
              <dt className="flex justify-start items-center gap-4">
                <span className="text-xs text-[var(--color-text-primary)] leading-[1]">
                  2025-00-00
                </span>
                <span className="text-xxs text-white px-2 py-1 bg-[var(--color-primary)] font-quaternary rounded-xl leading-[0.9] font-light tracking-[0.05rem]">
                  #勉強記録
                </span>
              </dt>
              <dd className="mt-4 text-md text-[var(--color-text-primary)] font-secondary leading-[1.6] tracking-[0.2rem] truncate w-[35vw]">
                日記のタイトルが入ります。
              </dd>
            </dl>
          </Link>
        </li>
        <li className="py-6 pl-8 pr-8 border-b-1 border-[var(--color-secondary-gray)]">
          <Link href="/nikki/1" className="block">
            <dl>
              <dt className="flex justify-start items-center gap-4">
                <span className="text-xs text-[var(--color-text-primary)] leading-[1]">
                  2025-00-00
                </span>
                <span className="text-xxs text-white px-2 py-1 bg-[var(--color-primary)] font-quaternary rounded-xl leading-[0.9] font-light tracking-[0.05rem]">
                  #勉強記録
                </span>
              </dt>
              <dd className="mt-4 text-md text-[var(--color-text-primary)] font-secondary leading-[1.6] tracking-[0.2rem] truncate w-[35vw]">
                日記のタイトルが入ります。
              </dd>
            </dl>
          </Link>
        </li>
      </ul>
    </>
  );
}
